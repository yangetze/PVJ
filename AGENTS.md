...
├── calculadora.html  # Calculator page — instant redirect stub to pagos.html#calculadora
├── pagos.html         # Payment reporting page — Pago Móvil / Transferencia data
├── donaciones.html   # Donations page — content only
├── mercancia.html     # Merch page — pro-fund shirt catalog + WhatsApp order form
├── styles.css        # All CSS: design system, layout, components, responsive
├── config.js          # Site-wide constants (SITE_URL, WHATSAPP_* numbers/messages, etc.)
...
├── mercancia.js       # mercancia.html JS: product catalog, order form, WhatsApp message builder
├── llms.txt            # Site summary for AI agents/crawlers (see SEO section below)
├── sitemap.xml         # XML sitemap — list indexable pages here
...
│   └── merch/          # Product images: modelo-front.png / modelo-back.png (back optional)
├── docs/
│   └── brand.md      # Brand guidelines: colors, tone of voice, copy examples
├── CLAUDE.md         # This file (Claude Code)
└── AGENTS.md          # Mirror of this file for other AI coding tools
```
...
| `config.js` | **Site-wide constants** (`SITE_URL`, `INSCRIPTION_URL`, `WHATSAPP_*`, etc.) — single source of truth |
...
| `mercancia.js` | mercancia.html JS: `merchModels` catalog data + rendering, order form, WhatsApp message builder |
...
1. Create `newpage.html` with standard `<head>` (linking `styles.css`), including the SEO meta tags block (title, description, keywords, canonical, Open Graph, Twitter Card) following the pattern in `mercancia.html`.
2. **Add the favicon link in `<head>`** — every HTML page must include:
   ```html
   <link rel="icon" type="image/x-icon" href="assets/favicon.ico">
   ```
3. Add an empty `<header></header>` near the top of `<body>`.
4. **Add a `<noscript>` nav fallback immediately after `<header></header>`**, copied from an existing page (`mercancia.html`, `donaciones.html`, `pagos.html`) and kept in sync with `NAV_ITEMS` in `components.js`. See [SEO & AI Crawler Guidelines](#seo--ai-crawler-guidelines) below — this is required, not optional.
5. Add an empty `<footer></footer>` near the bottom of `<body>`.
6. Load `components.js` (then your page-specific script) just before `</body>`.
7. Add the new page's nav entry to `NAV_ITEMS` in `components.js` — that's the **only** place to touch for nav/footer changes. Also add it to the `<noscript>` nav block on every page (not just the new one).
...
- **FAQ data** lives in the `faqData` array at the top of `main.js` (or `donacionesFaqData` in `donaciones.js`). To add or edit FAQ items, modify that array — the renderer builds the DOM automatically — **and** update the matching `<noscript>` fallback and `FAQPage` JSON-LD in the page's `<head>` (see [SEO & AI Crawler Guidelines](#seo--ai-crawler-guidelines)).
...
## SEO & AI Crawler Guidelines

All page content (nav, footer, FAQ, product catalog) is rendered client-side by JavaScript (`components.js`, `main.js`, `donaciones.js`, `mercancia.js`). Traditional search engine crawlers (Googlebot) execute JS, but many AI/LLM crawlers that answer questions or power "answer engines" (GPTBot, ClaudeBot, PerplexityBot, and similar) fetch raw HTML and do **not** execute JS. Without a fallback, they see effectively empty pages. This project addresses that with three mechanisms — all three are required together, not alternatives:

1. **`<noscript>` fallbacks** — every page has a `<noscript>` nav block right after `<header></header>` (mirrors `NAV_ITEMS` in `components.js`). Any section whose content comes from a JS data array (FAQ, product catalog) also has a `<noscript>` block with the same content inside the dynamic container, tagged with a `<!-- Keep in sync with X in Y.js -->` comment. When editing `faqData` (main.js), `donacionesFaqData` (donaciones.js), or `merchModels` (mercancia.js), **update the matching `<noscript>` block in the same commit.**
2. **JSON-LD structured data** — `<script type="application/ld+json">` blocks in `<head>`, read directly from raw HTML without executing JS. Current usage:
   - `index.html`: `Event`, `Organization`, `WebSite`, `FAQPage` (synced with `faqData`)
   - `donaciones.html`: `FAQPage` (synced with `donacionesFaqData`)
   - `mercancia.html`: `ItemList` of `Product` + `AggregateOffer` (synced with `merchModels`)

   Add the appropriate schema.org type for any new content-bearing page (e.g. another `FAQPage`, `Product`, or `Event`), and update it whenever the underlying data array changes.
3. **`llms.txt`** — a plain-text site summary at the repo root, in the emerging `llms.txt` convention for AI agents. Update it whenever a page's purpose, key facts (dates, prices, policies), or the page list itself changes.

Other SEO requirements for every page's `<head>`:
- `<title>`, `<meta name="description">`, `<meta name="keywords">`, `<link rel="canonical">`
- Open Graph (`og:type`, `og:url`, `og:title`, `og:description`, `og:image`, `og:locale`, `og:site_name`) and Twitter Card (`twitter:card`, `twitter:title`, `twitter:description`, `twitter:image`) tags
- `og:image`/`twitter:image` should point to a real, representative image when one exists (e.g. an actual product photo for `mercancia.html`), not always the generic logo
- New indexable pages must be added to `sitemap.xml`; pure redirect stubs (like `calculadora.html`) are excluded
- `robots.txt` allows all crawlers and points to `sitemap.xml` — don't restrict it without explicit instruction
